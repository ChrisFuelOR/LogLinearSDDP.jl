# # Computational experiments for a hydrothermal scheduling problem
...
# *	**loglinearSDDP**:
# Our version of SDDP from this repository. It is suited for uncertainty modeled by log-linear AR processes.
# *	**SDDP with linearized AR processes**:
# Uses standard SDDP from SDDP.jl. Requires to linearize the log-linear AR process beforehand.
# *	**Markov-chain SDDP**:
# Uses the Markov-chain SDDP (MC-SDDP) variant included in SDDP.jl. It requires a Markov-chain approximation (scenario lattice) of the uncertain inflows. This lattice is provided in the [MSPLib](https://github.com/bonnkleiford/MSPLib-Library) as well.
...
# *	`custom_model` (LOG-1 in the paper):		
# log-linear AR process with lag order 1
# *	`bic_model` (LOG-BIC in the paper):			
# log-linear AR process with lag order estimated using the Bayesian Information Criterion
# *	`fitted_model` (LIN-FIT in the paper):
# linearized AR process with lag order 1 fitted using our code
# *	`shapiro_model` (LIN-SHA in the paper):
# linearized AR process with lag order 1 and parameters from the literature
