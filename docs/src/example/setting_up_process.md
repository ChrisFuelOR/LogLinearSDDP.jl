...
*	For each reservoir system (N, S, NE, SE) and for each month, a log-linear AR model was fitted using the historical data (for more details, see [TODO]). The estimates are stored in folder `AutoregressivePreparation` and then `bic_model` (or `custom_model`) in some txt-files (for the linear case it is `LinearizedAutoregressivePreparation`). Each txt-file contains the months, the lag order, the process intercept, the process coefficients, the factor multiplied with the error term and the standard error from the estimation.
