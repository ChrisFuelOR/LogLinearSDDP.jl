...
Further note that running loglinearSDDP for our hydrothermal scheduling example (see [TODO]) requires an installation of Gurobi together with a valid Gurobi license. If this requirement is not satisfied, the solver has to be changed in the run-files and a few adjustments have to be made in the code.
...
Before using these steps, you have to make sure that the file_paths in `run_model.jl`, `run_model_linearized.jl`, `run_model_markov.jl`, `markov.jl` and `create_inflows_for_Markov_loglinear.jl` are set as intended. The places are highlighted by `TODO` in the code.
